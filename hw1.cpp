...
void render(Game *game)      // render means to PRESENT
{
    ...
    r.left = 10;
    r.center = 0;
    //ggprint8b(&r, 16, 0x00fffff, "my text salsa");
    ...
    r.bot = s->center.x -s->width -130; //fine
    r.left = s->center.y -s->width -400;
...
//dos
    ...
    r.bot = s->center.x - s->width-130;
    r.left= s->center.y - s->height-500;
    ...
    r.bot = s2->center.x - s2->width-162;
    r.left= s2->center.y - s2->height-475;
    ggprint16(&r,16,0x00ffff00,"THIRD");
    ...
    glVertex2i( w,-h);
    glEnd();
    r.bot = s2->center.x - s2->width-550;
    r.left= s2->center.y - s2->height + 25;
    ggprint12(&r,16,0x00ffff00,"FOURTH");
 ...
 r.bot = s2->center.x - s2->width-750;
 r.left= s2->center.y - s2->height + 300;
 ggprint12(&r,16,0x00ffff00,"FIFTH");
